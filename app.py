...
with left:
    ...
    if st.button("Send") and user_input.strip():
        # generate response (now returns structured results too)
        resp_text, interest, results = generate_response(user_input, st.session_state.context)

        # save and log
        ts = datetime.now().isoformat(timespec="seconds")
        st.session_state.history.append({"role": "user", "text": user_input, "interest": None, "ts": ts})
        st.session_state.history.append({"role": "bot", "text": resp_text, "interest": int(interest), "ts": ts})
        ...
        # log to DB (conversations table)
        try:
            log_conversation(user_input, resp_text, interest)
    ...
    if not st.session_state.history:
        st.info("Try queries: `show me burgers`, `I want vegan wraps`, `Any spicy vegetarian curry under $8?`")
    else:
    ...
    if st.session_state.last_results:
        for r in st.session_state.last_results:
            ...
            st.markdown(f"**{r[1]}** — *{r[2]}* — ${r[3]:.2f} — Spice {r[4]}/10")
            desc = (r[5] or "")
...
with right:
    ...
    # fetch last N conversation rows from DB for persistent analytics
    try:
    ...
    # Average interest (excluding zeros)
    ...
    # dietary mentions derived from product results
    st.markdown("**Dietary mentions (seen in results)**")
    st.write(", ".join(sorted(list(st.session_state.dietary_mentions))) or "None")

    # Small DB health check
